...
偏差的Jackknife估计值约为0.001，标准误差的Jackknife估计值约为0.05。
...
根据调整后最大的$R^2$选择二次模型。下面使用留一交叉验证来估计每个模型的预测误差。
...
二次模型拥有最小的预测误差，因此通过留一交叉验证仍然选择二次模型。
...
使用CvM检验大豆和亚麻籽p值不显著,没有足够的证据表明这两种分布之间存在显著差异。而向日葵和亚麻籽的结果在$\alpha=0.01$时显著，说明有足够的证据说明这两组分布不同。
